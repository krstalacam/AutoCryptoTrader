...
def select_buy_strategy_indicators(data):
    ...
    conditions.append((
            stoch_rsi_fallen(data) and
            sar_less_price(data) and
            # (not ema_is_lower(data, "ema_short")) and   # bu alttakinden farkli bu yakin zamana bakiyor alttaki daha eski zamana bakiyor
            # ema_is_higher(data, "ema_short") and
            # # buraya diger ema long falan eklemeli miyiz bilmiyorum eklesek de olur guzel olur ama baya kisitlar alinacak kriptolari belkide oyle olmali test etmelisin
            # ema_is_higher(data, "ema_long") and
            # ema_is_higher(data, "ema_longer")
            (not is_ema_50_dropping(data))
